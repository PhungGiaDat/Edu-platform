...
export const CourseDetail: React.FC = () => {
    ...
    if (isLoading) {
        return <div className="min-h-screen clay-bg-playful p-6 text-center text-xl font-black text-slate-700">{t('loadingCourse')}</div>;
    }

    if (!course || error) {
        return (
            <div className="min-h-screen clay-bg-playful p-6 text-center">
                <p className="text-xl font-black text-rose-600">{error || t('courseNotFound')}</p>
    ...
    const preview = course.catalogPreview.length > 0
        ...
        : [
            { label: t('sections'), value: String(course.lessons.length), color: 'sky' },
            { label: t('rewardXp'), value: String(stats.totalXp), color: 'amber' },
            { label: t('progress'), value: `${stats.percent}%`, color: 'violet' },
        ];
    const cta = enrollmentCta(course, locale, t('startLearning'));
        ...
        <div className="min-h-screen w-full max-w-[100vw] min-w-0 overflow-x-hidden clay-bg-playful pb-[calc(env(safe-area-inset-bottom)+12rem)] md:pb-10">
            <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
                ...
                <header className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px] lg:items-stretch">
                    <div className="rounded-[36px] border-4 border-white bg-gradient-to-br from-white via-sky-50 to-rose-50 p-5 shadow-[0_12px_0_rgba(91,141,239,0.14)] sm:p-7">
                        <div className="mb-4 inline-flex rounded-full bg-emerald-50 px-4 py-2 text-sm font-black text-emerald-600">
                            {t('age')} {course.age_range} - {courseTheme(course, locale)}
                        </div>
                ...
                <section className="mt-8 grid gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
                    <div>
                        <h2 className="mb-4 text-3xl font-black text-slate-800">{t('courseSections')}</h2>
                                        ...
                                        <div className="grid gap-4 sm:grid-cols-[72px_minmax(0,1fr)_auto] sm:items-center">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <div className="mt-2 flex flex-wrap gap-2 text-xs font-black">
                                                    <span className="rounded-full bg-yellow-50 px-3 py-1 text-yellow-700">{lesson.duration_minutes} {t('minute')}</span>
                                                    <span className="rounded-full bg-sky-50 px-3 py-1 text-sky-700">{lesson.vocabulary.length} {t('words')}</span>
                                                    {lesson.pronunciation && <span className="rounded-full bg-violet-50 px-3 py-1 text-violet-700">{t('say')}</span>}
                                                    {lesson.game && <span className="rounded-full bg-amber-50 px-3 py-1 text-amber-700">{t('game')}</span>}
                                                    <span className="rounded-full bg-emerald-50 px-3 py-1 text-emerald-700">{lesson.quiz.length} {t('quiz')}</span>
                                                </div>
                                            </div>
                                            <span className="clay-btn clay-btn-sm justify-center bg-gradient-to-br from-[#6EB9FF] to-[#B4E197] text-slate-800">
                                                {isComplete ? t('review') : t('startLesson')}
                                            </span>
                                        </div>
                                    </Link>
                                );
                    ...
                    <aside className="space-y-5">
                        <section className="rounded-[34px] border-4 border-white bg-gradient-to-br from-[#FFD93D] via-[#FFB36B] to-[#FF8E8E] p-5 shadow-[0_10px_0_rgba(248,113,113,0.18)]">
                            ...
                            >
                                {isStarting ? t('openingLesson') : cta.buttonLabel}
                            </button>
                        </section>

                        <section className="rounded-[34px] border-4 border-white bg-white/85 p-5 shadow-[0_10px_0_rgba(91,141,239,0.12)]">
                            <h2 className="text-2xl font-black text-slate-800">{t('studentVoices')}</h2>
