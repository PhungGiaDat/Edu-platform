...
export function registerCheckAssets(server: McpServer) {
  server.registerTool(
    'mindar_check_assets',
    {
...
Returns: per-file inventory + aggregate issue list.`,
    ...
    async (params: Input) => {
      const issues: Issue[] = [];
      const inventory: Array<{ path: string; ext: string; size: number }> = [];
      let totalGlbBytes = 0;
      let mindCount = 0;
      ...
      try {
        for await (const p of walk(params.buildDir)) {
          ...
          if (!['.mind', '.glb', '.gltf', '.jpg', '.jpeg', '.png', '.webp'].includes(ext)) continue;
          const stat = await fs.stat(p);
          inventory.push({ path: p, ext, size: stat.size });
          if (ext === '.glb' || ext === '.gltf') totalGlbBytes += stat.size;
          if (ext === '.mind') mindCount++;
        }
      } catch (err) {
