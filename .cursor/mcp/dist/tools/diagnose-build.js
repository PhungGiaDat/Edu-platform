...
export function registerDiagnoseBuild(server) {
    server.registerTool('mindar_diagnose_build', {
  ...
  - Per-frame performance`,
        ...
        try {
            for await (const p of walk(params.buildDir)) {
                if (path.extname(p).toLowerCase() === '.mind')
                    mindCount++;
            }
        }
        catch { }
...
//# sourceMappingURL=diagnose-build.js.map