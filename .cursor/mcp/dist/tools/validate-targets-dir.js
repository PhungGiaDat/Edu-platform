...
export function registerValidateTargetsDir(server) {
    server.registerTool('mindar_validate_targets_dir', {
...
Returns: per-image results + summary (PASS/WARN/FAIL counts).`,
        ...
        const results = await Promise.all(images.map(img => checkImage(path.join(params.dirPath, img))));
...
//# sourceMappingURL=validate-targets-dir.js.map