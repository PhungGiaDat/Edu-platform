...
async function unityRequest(endpoint: string, params?: Record<string, string>): Promise<UnityResponse> {
...
// Tool: unity_health
export function registerUnityHealth(server: McpServer) {
  server.registerTool(
    'unity_health',
    {
...
export function registerUnityBuild(server: McpServer) {
  server.registerTool(
    'unity_build_player',
    {
    ...
    async (params) => {
      try {
        ...
        if (params?.target) buildParams['buildTarget'] = params.target;
        if (params?.outputPath) buildParams['buildPath'] = params.outputPath;
...
export function registerUnityBuildStatus(server: McpServer) {
  server.registerTool(
    'unity_build_status',
    {
...
export function registerUnityDiagnostics(server: McpServer) {
  server.registerTool(
    'unity_diagnostics',
    {
...
// Tool: unity_packages
export function registerUnityPackages(server: McpServer) {
  server.registerTool(
    'unity_packages',
    {
...
export function registerUnityValidateTarget(server: McpServer) {
  server.registerTool(
    'unity_validate_target',
    {
...
// Tool: unity_scenes
export function registerUnityScenes(server: McpServer) {
  server.registerTool(
    'unity_scenes',
    {
      title: 'Unity Scenes',
