...
export const Login: React.FC = () => {
  ...
  const { login, enterGuestMode, isLoading, startPostLoginLoading, endPostLoginLoading, isPostLoginLoading } = useAuth();
  const { t } = useLocale();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      // Show Lexi overlay while navigating to authenticated area
  ...
  // Handle successful guest mode navigation
  ...
  // Sync isPostLoginLoading with isPostLoginLoading from auth context
  // Dismiss overlay when navigation completes (course list loads)
  const isLoadingCourses = isPostLoginLoading;

  // Auto-dismiss overlay when courses page starts rendering
  // (This handles the case where navigation completes before the loader is seen)
  useEffect(() => {
    // If post-login loading is active but we're on the login page,
    // the user might have navigated back. Clear it.
    if (isPostLoginLoading && window.location.pathname === '/login') {
      endPostLoginLoading();
    }
  }, [isPostLoginLoading, endPostLoginLoading]);
  ...
  return (
    <>
      ...
      {/* Lexi login loading overlay - appears after successful login */}
      <LexiLoginLoader
        isVisible={isLoadingCourses}
        onAnimationEnd={endPostLoginLoading}
      />
    </>
  );
};
