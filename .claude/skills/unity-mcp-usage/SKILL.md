...
description: Use the unity-mcp tools (48 tools, namespace `user-unityMCP`) to drive the Unity Editor from this project. Use when verifying package versions, reading compile errors, creating/modifying Unity scripts, taking scene screenshots, running EditMode/PlayMode tests, or making any mutation inside `mobile/unity/Assets/` that needs to compile against the actual Unity install. The MCP server is wired at project level (`e:\University\Graduted Project\Edu-platform\.claude\settings.local.json` + user-level `C:\Users\LENOVO\.cursor\mcp.json`) via `mcpforunityserver==10.1.2`. All 48 Unity MCP tools are allow-listed for Claude Code. Load only when an MCP call is the right path per the routing rule in `CLAUDE.md` (section 5); prefer Besty UnitySkills REST for most live Editor operations.
---
...
- **Project-level** (Claude Code): `e:\University\Graduted Project\Edu-platform\.claude\settings.local.json`
...
48 tools are allow-listed for Claude Code (all `mcp__UnityMCP__*`).
