---
description: Investigate and diagnose bugs and issues — does NOT implement fixes
...
Use the `debug` subagent (via the Agent tool with `subagent_type: "debug"`) to find the root cause of the issue, then analyze and explain the findings to the user.

**IMPORTANT: Do not implement the fix automatically.** Diagnose only — hand off to the `fix` subagent if the user wants to proceed.
...
[High-level suggestions for fix subagent — no code implementation here]
