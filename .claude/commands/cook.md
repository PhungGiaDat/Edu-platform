---
description: Implement a feature following the SDLC workflow
...
1. **Planning** — invoke `planner` and `researcher` subagents in parallel (via Agent tool)
2. **Design UI/UX** — load `ui-ux-pro-max` skill if UI is involved
3. **Development** — implement the feature on a feature branch
4. **Code Review** — invoke `reviewer` subagent, then `fix` subagent for any issues
5. **Testing** — invoke `tester` subagent, loop with `fix` until all tests pass
6. **Deployment** — invoke `devops` subagent to containerize and deploy
7. **Documentation** — invoke `documenter` subagent to update docs
