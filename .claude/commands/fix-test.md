...
Use the `debug` and `tester` subagents (via the Agent tool) to find the root cause of the failures, implement fixes, and repeat until all issues are resolved. Then delegate to the `reviewer` subagent for a final code review.
