...
Use the `planner` and `researcher` subagents (via the Agent tool) to read the GitHub Actions logs, analyze and identify the root causes of the failures, then provide a detailed plan for fixing them.
...
If confirmed, invoke the `fix` subagent to apply the changes, then run `/test` to verify.
