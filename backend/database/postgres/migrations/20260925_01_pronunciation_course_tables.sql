-- Pronunciation course schema for the existing PostgreSQL database.
-- The 20260906 migration cannot run on a database that already has flashcard pronunciation_attempts;
-- this migration keeps that table intact and uses pronunciation_course_attempts.
...
-- ============================================================
-- pronunciation_course_attempts  (separate from flashcard pronunciation_attempts)
-- Per-word attempt tracking with stars + XP
-- ============================================================
CREATE TABLE IF NOT EXISTS public.pronunciation_course_attempts (
...
CREATE INDEX IF NOT EXISTS idx_pronunciation_course_attempts_user_topic
    ON public.pronunciation_course_attempts (user_id, topic_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pronunciation_course_attempts_user_word
    ON public.pronunciation_course_attempts (user_id, word_id, stars DESC, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pronunciation_course_attempts_user_topic_word
    ON public.pronunciation_course_attempts (user_id, topic_id, word_id);
...
ALTER TABLE public.pronunciation_course_attempts ENABLE ROW LEVEL SECURITY;
