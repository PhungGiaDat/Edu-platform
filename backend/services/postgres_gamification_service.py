...
class PostgresGamificationService:
    ...
    def _result(event, replay: bool, current_streak: int = 0) -> dict[str, Any]:
        return {
            ...
            "total_xp_after": event["total_xp_after"],
            "level_after": event["level_after"],
            "xp_to_next_after": event["xp_to_next_after"],
            "level_up": False,
    ...
    async def apply_xp_event(
        ...
        *,
        ...
        user_exists = await connection.fetchval(
            "SELECT 1 FROM public.users WHERE id=$1", user_id
        )
        if not user_exists:
        ...
        if inserted is None:
            existing = await connection.fetchrow(
                "SELECT * FROM public.gamification_events WHERE user_id=$1 AND event_id=$2 FOR UPDATE",
                user_id,
                event_id,
            )
            if existing is None:
            ...
            if existing["status"] == "applied":
                ...
                streak_row = await connection.fetchrow(
                    "SELECT streak_days FROM public.user_gamification WHERE user_id=$1",
                    user_id,
                )
                return self._result(
                    existing,
                    replay=True,
                    current_streak=int(streak_row["streak_days"]) if streak_row else 0,
                )
        ...
        aggregate = await connection.fetchrow(
            "SELECT * FROM public.user_gamification WHERE user_id=$1 FOR UPDATE",
            user_id,
        )
        ...
        result = self._result(
            event, replay=False, current_streak=int(aggregate["streak_days"])
        )
    ...
    async def grant_badge_on_connection(
        connection, user_id: str, badge_id: str
    ) -> bool:
    ...
    async def add_xp(
        ...
        return await self.add_xp_with_event_id(
            user_id, f"legacy:{uuid4().hex}", action, metadata=metadata
        )
    ...
    async def get_user_stats(self, user_id: str) -> dict[str, Any]:
        ...
        value["stickers"] = [
            ...
            for item in await postgres_pool().fetch(
                "SELECT sticker_id AS id,name,rarity,image_url,earned_at FROM public.user_gamification_stickers WHERE user_id=$1",
                user_id,
            )
        ]
        return value
    ...
    async def get_leaderboard(
        self, period: str = "all", limit: int = 50
    ) -> list[dict[str, Any]]:
        from repositories.gamification_repository import GamificationRepository

        return await GamificationRepository().get_leaderboard(
            limit=limit, period=period
        )

    async def get_user_rank(
        self, user_id: str, period: str = "all"
    ) -> Optional[dict[str, Any]]:
        from repositories.gamification_repository import GamificationRepository

        return await GamificationRepository().get_user_rank(
    ...
    async def award_badge(self, user_id: str, badge_id: str) -> dict[str, Any]:
        row = await postgres_pool().fetchrow(
            """UPDATE public.user_gamification SET badges=(CASE WHEN badges ? $2 THEN badges ELSE badges || jsonb_build_array($2) END),updated_at=now()
               WHERE user_id=$1 RETURNING badges""",
            user_id,
            badge_id,
        )
        return {
            "success": bool(row),
            "badge_id": badge_id,
            "badges": list(row["badges"]) if row else [],
        }

    async def update_streak(self, user_id: str, activity_date: str) -> dict[str, Any]:
        row = await postgres_pool().fetchrow(
            "SELECT streak_days,longest_streak FROM public.user_gamification WHERE user_id=$1",
            user_id,
        )
        return {
            "success": bool(row),
            "current_streak": int(row["streak_days"]) if row else 0,
        }
    ...
    async def collect_sticker(self, user_id: str, sticker_id: str) -> dict[str, Any]:
        await postgres_pool().execute(
            "INSERT INTO public.user_gamification_stickers(user_id,sticker_id) VALUES($1,$2) ON CONFLICT DO NOTHING",
            user_id,
            sticker_id,
        )
    ...
    async def find_event(self, user_id: str, event_id: str) -> Optional[dict[str, Any]]:
        """Look up an XP event (idempotency pre-check before state mutation)."""
        row = await postgres_pool().fetchrow(
            "SELECT * FROM public.gamification_events WHERE user_id=$1 AND event_id=$2",
            user_id,
            event_id.strip() if event_id else "",
        )
    ...
    async def get_progress_report(self, user_id: str, days: int = 7) -> dict[str, Any]:
        return {"gamification": await self.get_user_stats(user_id), "days": days}

    async def _maybe_award_lesson_sticker(self, *args, **kwargs):
        return None

    async def _check_sticker_rewards(self, *args, **kwargs):
        return None
