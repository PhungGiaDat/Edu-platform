...
async def detect_qr_code(image_bytes: bytes) -> Optional[FlashcardSchema]:
    ...
    print(f"[DEBUG] 📸 Image bytes converted to numpy array of shape: ({', '.join(map(str, nparr.shape))})")
    image = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    print(f"[DEBUG] 🖼️ Image decoded with shape: {image.shape if image is not None else 'None'}")
    ...
    print(f"[DEBUG] 🔍 Decoded {len(decoded_objects)} QR codes from image")

    if not decoded_objects:
        return None  # Không tìm thấy QR
    print(f"[DEBUG] 📑 Found QR code with data: {decoded_objects[0].data.decode('utf-8')}")
    ...
    print(f"[DEBUG] 🏷️ AR Tag found: {ar_tag}")
    if ar_tag:
        ar_object = await ar_object_repo.get_by_tag(ar_tag)
        print(f"[DEBUG] 🧩 AR Object retrieved: {ar_object}")
        if ar_object:
