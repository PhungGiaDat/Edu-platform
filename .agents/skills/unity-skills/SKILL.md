...
### Three Modes (aligned with Codex permission modes)

> **Factory default:** a fresh install starts in **Auto**; an upgraded install (any pre-existing `UnitySkills_*` pref) starts in **Bypass**. It **never** defaults to Approval. The "Codex 类比" column below is only a mental model, **not** the factory default — always read `/health.currentMode` before acting.

| Mode | Codex 类比（心智对照，非默认） | FullAuto skill | Auto-detected NeverInSemi skill |
|---|---|---|---|
